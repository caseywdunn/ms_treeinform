...
#trees_7 <- threshold_7[[1]]
#dt_7 <- threshold_7[[3]]
#save(trees_7, file="data/revisions/trees_threshold-7.Rdata")
#save(dt_7, file="data/revisions/dt_threshold-7.Rdata")
load("data/revisions/dt_threshold-7.Rdata")
load("data/revisions/trees_threshold_7.Rdata")
...
wide_table<-data.frame(cbind(do.call(rbind,percentages),thresholds))
long_table<-melt(wide_table,id.vars=c("thresholds"))
long_table$value<-as.numeric(long_table$value)

test<-melt(data.frame(cbind(do.call(rbind,percentages),thresholds,total_percentage)),id.vars=c("thresholds"))
...
#ggplot(long_table,aes(thresholds,value)) + geom_point(aes(group=variable,color=variable)) + geom_point(data=data.frame(thresholds,total_percentage) + ggtitle("Percentage of reassigned genes by species vs threshold") + xlab("Threshold") + ylab("Percentage")

# also mean-variance plot
mean_var <- long_table %>% group_by(thresholds) %>% summarise(mean=mean(value),var=var(value))
```

```{r, eval=FALSE}
# could be a chunk somewhere, has some summary statistics...
gt_before <- mclapply(1:5606, function(x) read.tree(paste0("../revisions/before/",x,".newick")), mc.cores=cores)
before_counts <- count_all_transcripts(trees_before, cores)
after_counts <- count_all_transcripts(trees_7, cores)
...
dt <- c(dt_before, threshold_mix, threshold_5, dt_7, threshold_8)
l1 = length(dt_before)
