...
isoform_genes<-lapply(csv_files, function(x) read.csv(gttreyy file.path(TRINITY_PATH,x)))
...
cds_table <- data.frame(correct,diff_Y_same_X,thresholds)
ggplot(cds_table, aes(x=diff_Y_same_X,y=correct)) + geom_point() + geom_line() + ggtitle("Pairs of orrectly assigned transcripts vs\n pairs of transcripts incorrectly assigned to the same gene\n as threshold increases for 6 species of Drosophila") + xlab("Transcripts incorrectly assigned to the same gene") + ylab("Correctly assigned transcripts") + theme_classic() +
...
echino_lengths_lim = echino_lengths[which(echino_lengths<1)]
...
echino_cds_table <- data.frame(correct=echino_correct,diff_Y_same_X=echino_diff_Y_same_X,thresholds=echino_thresholds)
ggplot(echino_cds_table, aes(x=diff_Y_same_X,y=correct)) +
  geom_point() + geom_line() +
  ggtitle("Pairs of correctly assigned transcripts vs\n pairs of transcripts incorrectly assigned to the same gene\n as threshold increases for S. purpuratus") +
  xlab("Transcripts incorrectly assigned to the same gene") +
  ylab("Correctly assigned transcripts") + theme_classic() +
  ggrepel::geom_label_repel(aes(label = thresholds))
